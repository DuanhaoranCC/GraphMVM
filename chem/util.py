...
import numpy as np
from torch_geometric.utils import convert, to_networkx, remove_self_loops, contains_self_loops
...
class MaskAtom:
    def __init__(self, num_atom_type, num_edge_type, mask_rate, mask_rate1, mask_edge=True):
        """
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        """
        # data.rex = data.x.clone()
        edges, weights = creat_diff(data)
        edges = torch.tensor(np.array(list(edges)), dtype=torch.long)
        edges, weights = remove_self_loops(edge_index=edges, edge_attr=torch.FloatTensor(weights).unsqueeze(1))
        data1 = Data(x=data.x.clone(), edge_index=edges)
        data1.rex = data.x.clone()
        attr = torch.zeros(edges.size(1), 2)
        attr[:, 0] = 5
        data1.weight = weights
        data.weight = torch.ones(data.edge_index.size(1), 1).float()
        # attr = torch.cat([attr, weights], dim=1)
        # data.edge_attr = torch.cat([data.edge_attr, torch.ones(data.edge_index.size(1), 1).float()], dim=1)
        if data.edge_index.size(1) <= data1.edge_index.size(1):
            for id, edge in enumerate(data.edge_index.T):
                if edge in data1.edge_index.T:
                    attr[id] = data.edge_attr[id].clone()
        else:
            for id, edge in enumerate(data1.edge_index.T):
                if edge in data.edge_index.T:
                    attr[id] = data.edge_attr[id].clone()
        data1.edge_attr = attr.to(data.edge_attr.dtype)
        ########################################################################
        ...
        ##################################################################
        # num_atoms = data1.x.size()[0]
        # sample_size = int(num_atoms * self.mask_rate1 + 1)
        # masked_atom_indices = random.sample(range(num_atoms), sample_size)
        #
        # # create mask node label by copying atom feature of mask atom
        # mask_node_labels_list = []
        # for atom_idx in masked_atom_indices:
        #     mask_node_labels_list.append(data1.x[atom_idx].view(1, -1))
        # data1.mask_node_label = torch.cat(mask_node_labels_list, dim=0)
        # data1.masked_atom_indices = torch.tensor(masked_atom_indices)
        #
        # # ----------- graphMAE -----------
        # atom_type = F.one_hot(data1.mask_node_label[:, 0], num_classes=self.num_atom_type).float()
        # atom_chirality = F.one_hot(data1.mask_node_label[:, 1], num_classes=self.num_chirality_tag).float()
        # # data.node_attr_label = torch.cat((atom_type,atom_chirality), dim=1)
        # data1.node_attr_label = atom_type
        #
        # # modify the original node feature of the masked node
        # for atom_idx in masked_atom_indices:
        #     data1.x[atom_idx] = torch.tensor([self.num_atom_type, 0])
