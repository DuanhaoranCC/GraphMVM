...
class DataLoaderMaskingPred(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, mask_rate=0.0,
                 mask_rate1=0.0, mask_edge=0.0, **kwargs):
        self._transform = MaskAtom(num_atom_type=119, num_edge_type=5,
                                   mask_rate=mask_rate, mask_rate1=mask_rate1,
                                   mask_edge=mask_edge)
    ...
    def collate_fn(self, batches):
        ...
        batch1 = []
        for x in batches:
            data, data1 = self._transform(x)
            batch.append(data)
            batch1.append(data1)
        return BatchMasking.from_data_list(batch), BatchMasking.from_data_list(batch1)
