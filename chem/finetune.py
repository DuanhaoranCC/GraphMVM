...
def main(seed):
    ...
    parser.add_argument('--input_model_file', type=str, default='./checkpoints/_gin0.35_100.pth',
    ...
    if args.split == "scaffold":
        smiles_list = \
            pd.read_csv('/home/yhkj/dhr/KDD/chem/dataset/' + args.dataset + '/processed/smiles.csv', header=None)[
                0].tolist()
    ...
    # print(np.array(test_acc_list)[np.argmax(val_acc_list)])
    return np.array(test_acc_list)[np.argmax(val_acc_list)], test_acc_list[-1]
...
if __name__ == "__main__":
    ...
    print(np.array(A).std(), np.array(B).std())
    print(A)
    print(B)
