...
def load_graph_classification_dataset(dataset_name, deg4feat=False):
    ...
    dataset = TUDataset(dataset_name, raw_dir='./')
    graph, _ = dataset[0]
    ...
    for g, y in dataset:
        ...
        diff_graph.edata['diff_weight'] = torch.FloatTensor(diff_weight).float()
        dataset1.append((g, diff_graph, y))

    print(f"******** # Num Graphs: {len(dataset1)}, # Num Feat: {feature_dim}, # Num Classes: {num_classes} ********")

    return dataset1, (feature_dim, num_classes)
